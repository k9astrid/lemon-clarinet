...
import mc.clpz.base.event.impl.render.NameplateEvent;
import mc.clpz.base.module.impl.combat.KillAura;
import mc.clpz.base.module.impl.other.MurderMystery;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public <V extends EntityLivingBase, U extends LayerRenderer<V>> boolean addLayer3(LayerEnderDragonEyes layerEnderDragonEyes) {
    ...
    public <V extends EntityLivingBase, U extends LayerRenderer<V>> boolean addLayer5(LayerSheepWool layerSheepWool) {
        return this.layerRenderers.add((LayerRenderer<T>) layerSheepWool);
    }
    public <V extends EntityLivingBase, U extends LayerRenderer<V>> boolean addLayer6(LayerWolfCollar layerWolfCollar) {
        return this.layerRenderers.add((LayerRenderer<T>) layerWolfCollar);
    }
    public <V extends EntityLivingBase, U extends LayerRenderer<V>> boolean addLayer7(LayerSpiderEyes layerSpiderEyes) {
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                if (entity == Minecraft.getMinecraft().thePlayer) {
                    if (BaseClient.INSTANCE.getModuleManager().getModule("killaura").isEnabled() && KillAura.target != null || BaseClient.INSTANCE.getModuleManager().getModule("antiaim").isEnabled() || BaseClient.INSTANCE.getModuleManager().getModule("scaffold").isEnabled()) {
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        ...
        final Color visible = MurderMystery.murderers.contains(entitylivingbaseIn) ? new Color(255,0,0,255) : (new Color((BaseClient.INSTANCE.getFriendManager().isFriend(entitylivingbaseIn.getName()) || MurderMystery.gudpeople.contains(entitylivingbaseIn)) ? chams.visiblefriend.getValue() :chams.visible.getValue()));
        final Color hidden =MurderMystery.murderers.contains(entitylivingbaseIn) ? new Color(255, 128, 128,255) : (new Color((BaseClient.INSTANCE.getFriendManager().isFriend(entitylivingbaseIn.getName()) || MurderMystery.gudpeople.contains(entitylivingbaseIn)) ? chams.hiddenfriend.getValue() : chams.hidden.getValue()));
        ...
        if (flag || flag1) {
            ...
            if (BaseClient.INSTANCE.getModuleManager().getModuleClass(Chams.class).isEnabled() && chams.isValid(entitylivingbaseIn) && chams.colored.isEnabled()) {
