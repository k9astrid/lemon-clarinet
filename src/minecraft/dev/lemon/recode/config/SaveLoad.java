package dev.lemon.recode.config;
...
import dev.lemon.recode.setting.impl.BooleanSetting;
import dev.lemon.recode.setting.impl.KeybindSetting;
import dev.lemon.recode.setting.impl.ModeSetting;
import dev.lemon.recode.setting.impl.NumberSetting;

public class SaveLoad {
	/*
	 * eye bleeding braindamage made by clpz, pls make this better.
	 * */
	...
	public SaveLoad(String configFileName, boolean renderAndKeybinds) {
		...
		dir = new File(Minecraft.getMinecraft().mcDataDir, "LemonRecode");
		...
		File thing = new File(Minecraft.getMinecraft().mcDataDir, "versions/LemonRecode");
	...
	public void save() {
		...
		for(Module mod : Lemon.getModuleManager().getModules()) {
	...
	public void load(boolean renderAndKeybinds) {
		...
		for(String s : lines) {
			...
			if(s.toLowerCase().startsWith("module:")) {
				Module m = Lemon.getModuleManager().getModuleByName(args[1]);
				if(m != null) {
