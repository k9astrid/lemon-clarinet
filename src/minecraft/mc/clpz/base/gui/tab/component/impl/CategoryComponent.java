package mc.clpz.base.gui.tab.component.impl;
...
import mc.clpz.base.gui.tab.TabGUI;
import mc.clpz.base.gui.tab.component.Component;
import mc.clpz.base.module.Module;
import mc.clpz.base.module.impl.visuals.HUD;
import mc.clpz.base.utils.RenderUtil;
import mc.clpz.base.utils.font.Fonts;
...
public class CategoryComponent extends Component {
    ...
    public CategoryComponent(TabGUI mainTab, Module.Category category, float posX, float posY, float width, float height) {
        ...
        selectedModule = BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).isEmpty() ? null : BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).get(0);
    }

    @Override
    public void init() {
        float y = getPosY();
        for (Module module : BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category)) {
            moduleComponents.add(new ModuleComponent(this, module, getPosX() + getWidth() + 2, y, BaseClient.INSTANCE.getModuleManager().getLongestModInCategory(category) + 22, 12));
            y += 12;
        }
    ...
    public void onDraw(ScaledResolution scaledResolution) {
        for (ModuleComponent moduleComponent : moduleComponents) {
            ...
            moduleComponent.setWidth(BaseClient.INSTANCE.getModuleManager().getLongestModInCategory(category) + 22);
        }
        ...
        if (mainTab.isExtendedModule() && mainTab.getSelectedCategory() == getCategory()) {
            ...
            RenderUtil.drawBorderedRect(getPosX() + getWidth() + 2, getPosY(), BaseClient.INSTANCE.getModuleManager().getLongestModInCategory(category) + 22, 12 * moduleComponents.size(), 0.5f, 0xff000000, 0x00000000);
        }
    }
            ...
            switch (key) {
                case Keyboard.KEY_DOWN:
                    if (!mainTab.isExtendedValue() && !mainTab.isExtendedValueDynamic())
                        setSelectedModule(BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).get((BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).indexOf(getSelectedModule()) + 1) % BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).size()));
                    break;
                case Keyboard.KEY_UP:
                    if (!mainTab.isExtendedValue() && !mainTab.isExtendedValueDynamic())
                        setSelectedModule(BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).get((BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).indexOf(getSelectedModule()) - 1) < 0 ? BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).size() - 1 : (BaseClient.INSTANCE.getModuleManager().getModulesInCategory(category).indexOf(getSelectedModule()) - 1)));
                    break;
