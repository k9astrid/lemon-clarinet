package mc.clpz.base.gui.account.gui.impl;
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (key.getText().isEmpty() || BaseClient.INSTANCE.getAccountManager().getLastAlteningAlt() == null) return;
                BaseClient.INSTANCE.getAccountManager().setAlteningKey(key.getText());
                thread = new AccountLoginThread(BaseClient.INSTANCE.getAccountManager().getLastAlteningAlt().replaceAll(" ", ""), "ohareware");
                thread.run();
                BaseClient.INSTANCE.getAccountManager().save();
                break;
        }
