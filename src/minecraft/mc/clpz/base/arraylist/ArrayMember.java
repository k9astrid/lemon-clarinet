package mc.clpz.base.arraylist;
    ...
    public void draw(ArrayList<ArrayMember> arrayMembers, float startY, float prevY) {
        ...
        switch (hud.MODECOLORS.getValue()) {
            ...
            case CLIENTCOLOR:
                ...
                break;
            case WAVE:
                color = hud.color((int) (finishedY / (hud.font.isEnabled() ? hud.fontValue.getValue().getHeight() + 6 : mc.fontRendererObj.FONT_HEIGHT + 4)), (int) BaseClient.INSTANCE.getModuleManager().getMap().values().stream().filter(module1 -> module1.isEnabled() && !module1.isHidden()).count());
                break;
            case TESTRAINBOW:
        ...
        if (delete()) {
            ...
            BaseClient.INSTANCE.getArraylistManager().getArrayMembers().remove(this);
        }
    }
