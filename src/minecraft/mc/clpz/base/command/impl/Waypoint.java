        ...
        if (args.length > 1) {
            switch (args[1]) {
                case "help":
                    ...
                    break;
                case "reload":
                    BaseClient.INSTANCE.getWaypointManager().getWaypoints().clear();
                    BaseClient.INSTANCE.getWaypointManager().load();
                    Printer.print("Waypoints reloaded!");
                    break;
                case "clear":
                case "reset":
                    if (BaseClient.INSTANCE.getWaypointManager().getWaypoints().isEmpty()) {
                        Printer.print("Waypoints list is empty!");
                    } else {
                        BaseClient.INSTANCE.getWaypointManager().getWaypoints().clear();
                        Printer.print("Cleared waypoints list!");
                        BaseClient.INSTANCE.getWaypointManager().save();
                    }
                    break;
                case "list":
                    if (BaseClient.INSTANCE.getWaypointManager().getWaypoints().isEmpty()) {
                        ...
                        BaseClient.INSTANCE.getWaypointManager().getWaypoints().forEach(waypoint -> {
                ...
                case "add":
                    if (args.length == 6) {
                        if ((!NumberUtils.isNumber(args[2]) && NumberUtils.isNumber(args[3]) && NumberUtils.isNumber(args[4]) && NumberUtils.isNumber(args[5]))) {
                            if (!BaseClient.INSTANCE.getWaypointManager().isWaypoint(args[2], Minecraft.getMinecraft().getCurrentServerData().serverIP)) {
                                Printer.print("Waypoint " + args[2] + " (" + args[3] + ", " + args[4] + ", " + args[5] + ") has been added.");
                                BaseClient.INSTANCE.getWaypointManager().add(args[2], Double.valueOf(args[3]), Double.valueOf(args[4]), Double.valueOf(args[5]), Minecraft.getMinecraft().getCurrentServerData().serverIP, Minecraft.getMinecraft().thePlayer.dimension);
                                BaseClient.INSTANCE.getWaypointManager().save();
                            } else {
                        ...
                        if (!BaseClient.INSTANCE.getWaypointManager().isWaypoint(args[2], Minecraft.getMinecraft().getCurrentServerData().serverIP)) {
                            ...
                            BaseClient.INSTANCE.getWaypointManager().add(args[2], (int) Minecraft.getMinecraft().thePlayer.posX, (int) Minecraft.getMinecraft().thePlayer.posY, (int) Minecraft.getMinecraft().thePlayer.posZ, Minecraft.getMinecraft().getCurrentServerData().serverIP, Minecraft.getMinecraft().thePlayer.dimension);
                            BaseClient.INSTANCE.getWaypointManager().save();
                        } else {
                            Printer.print(args[2] + " is already a waypoint!");
                        }
                ...
                case "remove":
                    if (BaseClient.INSTANCE.getWaypointManager().isWaypoint(args[2], Minecraft.getMinecraft().getCurrentServerData().serverIP)) {
                        ...
                        BaseClient.INSTANCE.getWaypointManager().remove(args[2], Minecraft.getMinecraft().getCurrentServerData().serverIP);
                        BaseClient.INSTANCE.getWaypointManager().save();
                    } else {
