...
import mc.clpz.base.module.impl.combat.AntiBot;
...
public class TargetStrafe extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        if (!BaseClient.INSTANCE.getModuleManager().getModule("speed").isEnabled() || (!getMc().gameSettings.keyBindJump.isKeyDown() && TargetStrafe.spaceOnly.isEnabled())) {target = null; set = false;return;}
        target = getTarget();
        if (target != null) {
    ...
    private void drawCirle(Entity entity, Color color, float partialTicks) {
        ...
        GL11.glBegin(GL11.GL_LINE_STRIP);
        {
            ...
            for (Vec3 pos : posArrayList) {
                GL11.glColor3d(BaseClient.INSTANCE.getModuleManager().getModule("speed").isEnabled() && posArrayList.indexOf(pos) == index ? 0.15f : r, BaseClient.INSTANCE.getModuleManager().getModule("speed").isEnabled() && posArrayList.indexOf(pos) == index ? 0.15f : g, BaseClient.INSTANCE.getModuleManager().getModule("speed").isEnabled() && posArrayList.indexOf(pos) == index ? 1 : b);
