    ...
    public void onRender3D(Render3DEvent event) {
        for (Entity entity : getMc().theWorld.getLoadedEntityList()) {
            if (entity instanceof EntityLivingBase && isValid((EntityLivingBase)entity)) {
                trace(entity, width.getValue(), BaseClient.INSTANCE.getFriendManager().isFriend(entity.getName()) ? new Color(32, 128, 255) : new Color(255,255,255), Minecraft.getMinecraft().timer.renderPartialTicks);
            }
        }
    }
