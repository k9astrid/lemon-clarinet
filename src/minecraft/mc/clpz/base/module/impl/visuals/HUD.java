...
import mc.clpz.base.arraylist.ArrayMember;
import mc.clpz.base.event.bus.Handler;
import mc.clpz.base.event.impl.input.KeyInputEvent;
import mc.clpz.base.event.impl.render.Render2DEvent;
import mc.clpz.base.gui.tab.TabGUI;
import mc.clpz.base.module.Module;
import mc.clpz.base.utils.RenderUtil;
import mc.clpz.base.utils.font.MCFontRenderer;
import mc.clpz.base.utils.value.impl.*;
...
public class HUD extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        String watermark = "Base" + ChatFormatting.GRAY + "Client epik haka edition";
        ...
        if (!animations.isEnabled()) {
            float posY = 3;
            final ArrayList<Module> mods = new ArrayList<>(BaseClient.INSTANCE.getModuleManager().getModuleMap().values());
            ...
            for (Module module : mods) {
                ...
                switch (MODECOLORS.getValue()) {
                    ...
                    case CLIENTCOLOR:
                        ...
                        break;
                    case WAVE:
                        color = color((int) (posY / (font.isEnabled() ? fontValue.getValue().getHeight() + 6 : getMc().fontRendererObj.FONT_HEIGHT + 4)), (int) BaseClient.INSTANCE.getModuleManager().getMap().values().stream().filter(module1 -> module1.isEnabled() && !module1.isHidden()).count());
                        break;
                }
    ...
    private void drawNotifications(ScaledResolution scaledResolution) {
        ...
        for (int i = 0; i < BaseClient.INSTANCE.getNotificationManager().getNotifications().size(); i++) {
            BaseClient.INSTANCE.getNotificationManager().getNotifications().get(i).draw(y);
            y -= 26;
        }
    }

    private void drawArraylist() {
        for (Module module : BaseClient.INSTANCE.getModuleManager().getModuleMap().values()) {
            if (!module.isEnabled() || module.isHidden() || BaseClient.INSTANCE.getArraylistManager().isArrayMember(module))
                continue;
            BaseClient.INSTANCE.getArraylistManager().addArray(module);
        }
        float posY = 3;
        final ArrayList<ArrayMember> arrayMembers = new ArrayList<>(BaseClient.INSTANCE.getArraylistManager().getArrayMembers());
