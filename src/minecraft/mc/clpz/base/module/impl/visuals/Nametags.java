...
import mc.clpz.base.utils.value.impl.ColorValue;
import mc.clpz.base.utils.value.impl.EnumValue;
import mc.clpz.base.utils.value.impl.FontValue;
...
public class Nametags extends Module {
            ...
            if (entity instanceof EntityLivingBase) {
                ...
                if (isValid(ent) && entity.getUniqueID() != getMc().thePlayer.getUniqueID() && RenderUtil.isInViewFrustrum(ent)) {
                    ...
                    if (position != null) {
                        ...
                        final String nametext = (distance.isEnabled() ? "(" + Math.round(getMc().thePlayer.getDistance(ent.posX, ent.posY, ent.posZ)) + "m) " : "") + (((BaseClient.INSTANCE.getFriendManager().isFriend(ent.getName()) && BaseClient.INSTANCE.getFriendManager().getFriend(ent.getName()).getAlias() != null) ? BaseClient.INSTANCE.getFriendManager().getFriend(ent.getName()).getAlias() : (stripColorCodes.isEnabled() ?StringUtils.stripControlCodes(ent.getName()):ent.getDisplayName().getUnformattedText()))) + (health.isEnabled() ? healthtext : "");
    ...
    private int getNameColor(EntityLivingBase ent) {
        if (BaseClient.INSTANCE.getFriendManager().isFriend(ent.getName())) return new Color(122, 190, 255).getRGB();
