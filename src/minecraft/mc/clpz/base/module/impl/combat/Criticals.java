package mc.clpz.base.module.impl.combat;
...
import mc.clpz.base.event.bus.*;
import mc.clpz.base.event.impl.player.*;
...
public class Criticals extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (Mode.getValue() == mode.CUBECRAFT) {
            if (event.isPre()) {
                if (!Mouse.isButtonDown(0) && getMc().thePlayer.motionY < 0 && getMc().thePlayer.onGround && !BaseClient.INSTANCE.getModuleManager().getModule("scaffold").isEnabled() && getMc().thePlayer.onGround && !BaseClient.INSTANCE.getModuleManager().getModule("speed").isEnabled() && !BaseClient.INSTANCE.getModuleManager().getModule("flight").isEnabled() && !getMc().gameSettings.keyBindJump.isKeyDown() && getMc().thePlayer.fallDistance == 0) {
        ...
        if (Mode.getValue() == mode.HYPIXELHVH) {
            if (event.isPre()) {
                if (getMc().thePlayer.motionY < 0 && isBlockUnder() && getMc().thePlayer.onGround && !BaseClient.INSTANCE.getModuleManager().getModule("speed").isEnabled() && !BaseClient.INSTANCE.getModuleManager().getModule("flight").isEnabled() && !getMc().gameSettings.keyBindJump.isKeyDown() && getMc().thePlayer.fallDistance == 0) {
                    event.setOnGround(false);
