...
import mc.clpz.base.utils.MathUtils;
        ...
        if (!event.isSending()) {
            if (event.getPacket() instanceof S08PacketPlayerPosLook) {
                ...
                if (autodisable.getValue() && (!(getMc().theWorld.getBlockState(new BlockPos(getMc().thePlayer.posX, getMc().thePlayer.posY - 1.0, getMc().thePlayer.posZ)).getBlock() instanceof BlockAir) || !(getMc().theWorld.getBlockState(new BlockPos(getMc().thePlayer.posX, getMc().thePlayer.posY - 2.0, getMc().thePlayer.posZ)).getBlock() instanceof BlockAir))) {
                    if (BaseClient.INSTANCE.getModuleManager().getModule("speed").isEnabled() || BaseClient.INSTANCE.getModuleManager().getModule("flight").isEnabled() || BaseClient.INSTANCE.getModuleManager().getModule("longjump").isEnabled()) {
                        BaseClient.INSTANCE.getModuleManager().getModule("speed").setEnabled(false);
                        BaseClient.INSTANCE.getModuleManager().getModule("flight").setEnabled(false);
                        BaseClient.INSTANCE.getModuleManager().getModule("longjump").setEnabled(false);
                        BaseClient.INSTANCE.getNotificationManager().addNotification("Lag back/Teleport! turned off some mods", 2500);
                        return;
                    }
                }
                ...
                if (message.toLowerCase().contains(getMc().getSession().getUsername().toLowerCase()) || !strict.getValue()) {
                    if (autoNoU.isEnabled()) {
                        for (String noUMatch : noUMatches) {
                            ...
                            BaseClient.INSTANCE.getNotificationManager().addNotification("Detected a Salty Noob, auto no u", 2500);
                            break;
                        }
                    }
                    if (suspect.isEnabled()) {
                        for (String hackMatch : hackMatches) {
                            ...
                            BaseClient.INSTANCE.getNotificationManager().addNotification("Suspected of cheating! turn it down buddy!", 2500);
                            break;
                        }
                    }
                }
                if (message.equalsIgnoreCase("ground items will be removed in ")) {
                    BaseClient.INSTANCE.getNotificationManager().addNotification("Ground items will be gone in " + message.split("be removed in ")[1], 2500);
                }
            }
        }
    }
